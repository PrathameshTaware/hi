...
import { Mic, MicOff, Volume2, ArrowLeft, Send, AlertCircle, Activity } from 'lucide-react'
...
export default function VoicePage() {
  ...
  const processTextWithStreaming = async (text: string) => {
    if (!text.trim()) return
    ...
    try {
      await apiClient.streamTextProcessing(text, 'voice_ui_user', (event) => {
        switch (event.type) {
          case 'start':
            setVoiceState(prev => ({ ...prev, currentStep: 'Starting AI processing...', progress: 0 }))
            break
            
          case 'node_start':
            const stepMessage = getStepMessage(event.node || '')
            setVoiceState(prev => ({ 
              ...prev, 
          ...
          case 'node_complete':
            setStreamingSteps(prev => prev.map(step => 
              step.step === event.node 
                ? { ...step, duration: 0.5 } // Mock duration
                : step
            ))
            break
          ...
          case 'complete':
            if (event.result?.success) {
              handleProcessingResult(event.result, text)
            } else {
              throw new Error(event.result?.error || 'Processing failed')
            }
            break
  ...
  const getStepMessage = (step: string): string => {
    const messages = {
      'safety_check': '🛡️ Checking content safety...',
      'intent_router': '🧠 Analyzing your intent...',
      'retrieve_context': '📚 Retrieving relevant information...',
      'generate_response': '✨ Generating AI response...',
      'post_process': '🎯 Finalizing output...'
    }
    return messages[step as keyof typeof messages] || `Processing ${step}...`
  }
  ...
  const handleTextSubmit = (e: React.FormEvent) => {
    ...
    if (textInput.trim() && !voiceState.isProcessing) {
      processTextWithStreaming(textInput)
      setTextInput('')
    }
  }

  const testStreamingDemo = async () => {
    setVoiceState(prev => ({ 
      ...prev, 
      isProcessing: true, 
      isStreaming: true,
    ...
    try {
      await apiClient.streamDemo((event) => {
        if (event.type === 'step') {
          setVoiceState(prev => ({ 
            ...prev, 
          ...
          if (event.result) {
            handleProcessingResult(event.result, 'Demo streaming test')
          }
        }
      })
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-rural-green">
      ...
      <div className="max-w-4xl mx-auto p-4 pt-8">
        ...
        <LoadingOverlay isLoading={voiceState.isProcessing && !voiceState.isStreaming} text="Processing your request...">
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 mb-6">
              ...
              {voiceState.isStreaming && (
                <div className="text-cyber-blue text-lg mb-4">
                  <div className="flex items-center justify-center space-x-2 mb-2">
                    ...
                    <span>{voiceState.currentStep}</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2 mb-2">
                    <div 
                      className="bg-cyber-blue h-2 rounded-full transition-all duration-300"
                      style={{ width: `${voiceState.progress * 100}%` }}
                    ></div>
                  </div>
                  <span className="text-sm text-gray-300">
                    {Math.round(voiceState.progress * 100)}% complete
                  </span>
                </div>
              )}

              {voiceState.isProcessing && !voiceState.isStreaming && (
                <div className="text-cyber-blue text-lg mb-4">
            ...
            {/* Streaming Steps Visualization */}
            {streamingSteps.length > 0 && (
              <div className="mb-6 bg-black/20 rounded-lg p-4">
                <h3 className="text-white font-semibold mb-3">AI Processing Steps:</h3>
                <div className="space-y-2">
                  {streamingSteps.map((step, index) => (
                    <div key={index} className="flex items-center space-x-3">
                      <div className={`w-3 h-3 rounded-full ${
                        step.duration ? 'bg-safe-green' : 'bg-rural-gold animate-pulse'
                      }`}></div>
                      <span className="text-gray-200 text-sm">{step.message}</span>
                      {step.duration && (
            ...
            {/* Demo Streaming Button */}
            <div className="flex justify-center mb-6">
              <button
                onClick={testStreamingDemo}
                disabled={voiceState.isProcessing || !voiceState.isConnected}
                className="bg-cyber-blue hover:bg-blue-600 disabled:bg-gray-500 disabled:cursor-not-allowed text-white px-6 py-2 rounded-lg font-semibold transition-colors flex items-center space-x-2"
              >
                <Activity className="h-4 w-4" />
                <span>Test Streaming Demo</span>
              </button>
            </div>
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="text-xs text-gray-400">
                      ...
                      {message.intent && (
                        <span className="ml-2">Intent: {message.intent}</span>
                      )}
                    </div>
                  </div>
